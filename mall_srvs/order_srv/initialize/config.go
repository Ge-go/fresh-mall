...
func InitConfig() {
	configFileName := "order_srv/config-debug.yaml" // debug环境
	pro := GetEnvInfo("FRESH_MALL_PRO")                 // 生产环境
	if pro {
		configFileName = "order_srv/config-pro.yaml" //生产环境
	} else if GetEnvInfo("FRESH_TEST") { //test环境 for windows
		configFileName = "order_srv/config-test.yaml"
	}
